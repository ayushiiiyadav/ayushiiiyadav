<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=b8a9e3&height=180&section=header&text=Hey%2C%20I'm%20Ayushi%20&fontSize=40&fontColor=fdf6ec&fontAlignY=60&desc=Web%20Developer%20%C2%B7%20Tech%20Enthusiast%20%C2%B7%20Final%20Year%20Student&descSize=16&descAlignY=78&descFontColor=fdf6ec&animation=fadeIn" width="100%" />
  <br/><br/>

  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=15&pause=1000&color=b8a9e3&center=true&vCenter=true&width=600&lines=React+%7C+Next.js+%7C+TypeScript+%7C+Node.js;Building+full-stack+apps%2C+one+commit+at+a+time;Currently+exploring+RAG+%26+AI-powered+apps+%F0%9F%A4%96;Always+learning%2C+always+building+%F0%9F%9A%80" alt="Typing SVG" />
  <br/><br/>

  <img src="https://komarev.com/ghpvc/?username=ayushiiiyadav&color=b8a9e3&style=for-the-badge&label=PROFILE+VIEWS" alt="Profile Views"/>
</div>

---

## 🙋‍♀️ About Me

> *I build things for the web — and learn from every challenge along the way.*

I'm a **final year Computer Science student** at **Shri Ram Swaroop Memorial University**, passionate about full-stack web development. I love turning ideas into clean, user-friendly applications and exploring AI integration.

I am **actively seeking internships**, freelance projects, and full-time opportunities in web development.

```ts
const ayushi = {
  name: "Ayushi Yadav",
  handle: "@ayushiiiyadav",
  role: "Web Developer & Tech Enthusiast",
  location: "India",
  university: "Shri Ram Swaroop Memorial University",
  stack: ["React", "Next.js", "TypeScript", "Node.js", "FastAPI", "MongoDB"],
  currently: "Building full-stack apps & exploring AI integration",
  openTo: ["internships", "freelance projects", "full-time roles"],
};
```

---

## 🚀 Featured Projects

All projects shown in full below — click a title to collapse it if you want a shorter scroll ⬇️

<details open>
<summary><b>🤖 Nova AI — RAG-Powered Chatbot</b></summary>
<br/>

A Groq-powered chatbot upgraded with a Retrieval-Augmented Generation pipeline so it can answer from real documents, not just model memory.

**Tech Stack**

![Python](https://img.shields.io/badge/Python-b8a9e3?style=flat-square&logo=python&logoColor=fdf6ec)
![Groq](https://img.shields.io/badge/Groq_API-b8a9e3?style=flat-square&logo=groq&logoColor=fdf6ec)
![ChromaDB](https://img.shields.io/badge/ChromaDB-b8a9e3?style=flat-square&logo=databricks&logoColor=fdf6ec)
![Streamlit](https://img.shields.io/badge/Streamlit-b8a9e3?style=flat-square&logo=streamlit&logoColor=fdf6ec)

- LLM: Groq API with a selectable multi-model setup
- RAG: ChromaDB + sentence-transformers for embedding & retrieval
- UI: custom dark theme with an animated starfield canvas

[Repo →](https://github.com/ayushiiiyadav/ai-chatbot)

</details>

<details open>
<summary><b>🎯 Magic Quiz — Interactive Quiz App for Kids</b></summary>
<br/>

My first full project built from scratch — five quiz categories designed for primary schoolers, with game-like feedback loops to keep them engaged.

**Tech Stack**

![React](https://img.shields.io/badge/React-b8a9e3?style=flat-square&logo=react&logoColor=fdf6ec)
![Vite](https://img.shields.io/badge/Vite-b8a9e3?style=flat-square&logo=vite&logoColor=fdf6ec)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-b8a9e3?style=flat-square&logo=tailwind-css&logoColor=fdf6ec)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-b8a9e3?style=flat-square&logo=framer&logoColor=fdf6ec)

- 5 quiz categories with lives/hearts system
- Streak bonuses + confetti animations on correct answers
- Deployed on Vercel via GitHub integration

[Repo →](https://github.com/ayushiiiyadav/magic-quiz)

</details>

<details open>
<summary><b>🌐 Portfolio Website</b></summary>
<br/>

A single consolidated `index.html` with embedded CSS/JS — no framework, just hand-built animation.

**Tech Stack**

![HTML5](https://img.shields.io/badge/HTML5-b8a9e3?style=flat-square&logo=html5&logoColor=fdf6ec)
![CSS3](https://img.shields.io/badge/CSS3-b8a9e3?style=flat-square&logo=css3&logoColor=fdf6ec)
![JavaScript](https://img.shields.io/badge/JavaScript-b8a9e3?style=flat-square&logo=javascript&logoColor=fdf6ec)

- Animated star canvas background + custom cursor
- Scroll-reveal animations on project cards
- Deployed on Vercel via GitHub integration

[Repo →](https://github.com/ayushiiiyadav/Portfolio) · [Live →](https://portfoliopage-inky.vercel.app)

</details>

<details open>
<summary><b>🛒 E-Commerce Site</b></summary>
<br/>

A full storefront build covering the core shopping flow — product listings, cart, and checkout — built without a framework to strengthen fundamentals in vanilla JS.

**Tech Stack**

![HTML5](https://img.shields.io/badge/HTML5-b8a9e3?style=flat-square&logo=html5&logoColor=fdf6ec)
![CSS3](https://img.shields.io/badge/CSS3-b8a9e3?style=flat-square&logo=css3&logoColor=fdf6ec)
![JavaScript](https://img.shields.io/badge/JavaScript-b8a9e3?style=flat-square&logo=javascript&logoColor=fdf6ec)

- Product listing pages with cart add/remove logic
- Persistent cart state across page navigation
- Deployed on Vercel via GitHub integration

[Repo →](https://github.com/ayushiiiyadav/E-Commerce-Website) · [Live →](https://zivara-gamma.vercel.app)

</details>

<details open>
<summary><b>✅ To-Do List — Kawaii Task Manager</b></summary>
<br/>

A basic HTML to-do list refactored into a multi-file app with a cute, mascot-driven aesthetic — built to practice breaking a single-file project into organized components.

**Tech Stack**

![HTML5](https://img.shields.io/badge/HTML5-b8a9e3?style=flat-square&logo=html5&logoColor=fdf6ec)
![CSS3](https://img.shields.io/badge/CSS3-b8a9e3?style=flat-square&logo=css3&logoColor=fdf6ec)
![JavaScript](https://img.shields.io/badge/JavaScript-b8a9e3?style=flat-square&logo=javascript&logoColor=fdf6ec)

- LocalStorage persistence for tasks across sessions
- Filter views (all / active / completed)
- Confetti celebration + mascot system on task completion

[Repo →](https://github.com/ayushiiiyadav/TO_DO_LIST)

</details>

---

## ✨ My Coding Journey

- Started with HTML, CSS & JavaScript
- Built multiple full-stack and AI-powered projects, from quiz apps to RAG-based chatbots
- Currently focusing on full-stack apps with real backends and AI/RAG integration

---

## 🛠️ Tech Stack
### ⚡ Frontend
![React](https://img.shields.io/badge/React-b8a9e3?style=for-the-badge&logo=react&logoColor=fdf6ec)
![Next.js](https://img.shields.io/badge/Next.js-b8a9e3?style=for-the-badge&logo=next.js&logoColor=fdf6ec)
![TypeScript](https://img.shields.io/badge/TypeScript-b8a9e3?style=for-the-badge&logo=typescript&logoColor=fdf6ec)
![JavaScript](https://img.shields.io/badge/JavaScript-b8a9e3?style=for-the-badge&logo=javascript&logoColor=fdf6ec)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-b8a9e3?style=for-the-badge&logo=tailwind-css&logoColor=fdf6ec)
![HTML5](https://img.shields.io/badge/HTML5-b8a9e3?style=for-the-badge&logo=html5&logoColor=fdf6ec)
![CSS3](https://img.shields.io/badge/CSS3-b8a9e3?style=for-the-badge&logo=css3&logoColor=fdf6ec)

### 🔧 Backend
![Node.js](https://img.shields.io/badge/Node.js-b8a9e3?style=for-the-badge&logo=node.js&logoColor=fdf6ec)
![FastAPI](https://img.shields.io/badge/FastAPI-b8a9e3?style=for-the-badge&logo=fastapi&logoColor=fdf6ec)
![Express.js](https://img.shields.io/badge/Express.js-b8a9e3?style=for-the-badge&logo=express&logoColor=fdf6ec)
![REST API](https://img.shields.io/badge/REST_API-b8a9e3?style=for-the-badge&logo=postman&logoColor=fdf6ec)
![JWT](https://img.shields.io/badge/JWT-b8a9e3?style=for-the-badge&logo=JSON%20web%20tokens&logoColor=fdf6ec)

### 🤖 AI / ML
![Groq](https://img.shields.io/badge/Groq_API-b8a9e3?style=for-the-badge&logo=groq&logoColor=fdf6ec)
![ChromaDB](https://img.shields.io/badge/ChromaDB-b8a9e3?style=for-the-badge&logo=databricks&logoColor=fdf6ec)
![Sentence Transformers](https://img.shields.io/badge/Sentence_Transformers-b8a9e3?style=for-the-badge&logo=huggingface&logoColor=fdf6ec)
![Streamlit](https://img.shields.io/badge/Streamlit-b8a9e3?style=for-the-badge&logo=streamlit&logoColor=fdf6ec)
![Python](https://img.shields.io/badge/Python-b8a9e3?style=for-the-badge&logo=python&logoColor=fdf6ec)

### 🗄️ Databases
![MongoDB](https://img.shields.io/badge/MongoDB-b8a9e3?style=for-the-badge&logo=mongodb&logoColor=fdf6ec)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-b8a9e3?style=for-the-badge&logo=postgresql&logoColor=fdf6ec)
![SQLite](https://img.shields.io/badge/SQLite-b8a9e3?style=for-the-badge&logo=sqlite&logoColor=fdf6ec)
![MySQL](https://img.shields.io/badge/MySQL-b8a9e3?style=for-the-badge&logo=mysql&logoColor=fdf6ec)

### ☁️ Cloud & DevOps
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-b8a9e3?style=for-the-badge&logo=github-actions&logoColor=fdf6ec)
![Vercel](https://img.shields.io/badge/Vercel-b8a9e3?style=for-the-badge&logo=vercel&logoColor=fdf6ec)
![Git](https://img.shields.io/badge/Git-b8a9e3?style=for-the-badge&logo=git&logoColor=fdf6ec)

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://raw.githubusercontent.com/ayushiiiyadav/ayushiiiyadav/main/profile-summary-card-output/github/stats.svg" />
  <img src="https://raw.githubusercontent.com/ayushiiiyadav/ayushiiiyadav/main/profile-summary-card-output/github/repos-per-language.svg" />
</div>

<div align="center">
  <img src="https://raw.githubusercontent.com/ayushiiiyadav/ayushiiiyadav/main/profile-summary-card-output/github/most-commit-language.svg" />
</div>
---

## 🐍 Contribution Snake

<div align="center">
  <img src="https://raw.githubusercontent.com/ayushiiiyadav/ayushiiiyadav/output/github-contribution-grid-snake.svg" alt="Contribution snake animation" width="90%"/>
</div>

---

## 🌐 Let's Connect

<div align="center">
  <a href="https://github.com/ayushiiiyadav">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
  </a>
  <a href="https://www.linkedin.com/in/ayushi-yadav-61a891311">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="mailto:ayu6241997@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
  </a>

  <br/><br/>
  <strong>I am actively looking for internships and opportunities.</strong>
  <br/><br/>

  <img src="https://capsule-render.vercel.app/api?type=waving&color=b8a9e3&height=100&section=footer" width="100%"/>
</div>
